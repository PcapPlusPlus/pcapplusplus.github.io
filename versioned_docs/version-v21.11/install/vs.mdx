...
| __`-z`__ , __`--zstd-sdk`__      | Path to Zstd SDK directory. This is an optional parameter that is required only if using [Zstd for PCAPNG streaming compression](../features#readingwriting-pcapng-files-with-compression) |
