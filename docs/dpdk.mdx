...
Once the DPDK build is completed successfully, run PcapPlusPlus configuration script (`configure-linux.sh`) to build PcapPlusPlus with DPDK. Please refer to the [configuration instructions *LINK*](./install/build-source/linux#configuration) to get more details.
...
- DPDK example applications: [DpdkExample-FilterTraffic *LINK*](./examples#dpdkexample-filtertraffic), [DpdkBridge *LINK*](./examples#dpdkbridge), [KniPong *LINK*](./examples#knipong)
