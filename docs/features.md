...
- Write compressed PCAPNG files using [zstd](https://github.com/facebook/zstd) compression in real time (OPTIONAL requires building with zstd)
...
Zstd streaming compression is only supported when working with pcapng files. To enable this feature you must build PcapPlusPlus with zstd support. For more guidance on building PcapPlusPlus see the build instructions [here]({{ site.baseurl }}/docs/install)
