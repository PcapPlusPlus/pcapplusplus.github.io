...
You can find out more information in the [API documentation *LINK*](./api), [tutorial pages](./tutorials/intro) and browse through the code of the [example apps *LINK*]/docs/examples).
...
For more information please refer to the [Read/Write Pcap Files tutorial](./tutorials/read-write-pcap), look at the [API documentation *LINK*](./api) or browse through the code of the [example apps *LINK*](./examples).

### Reading/Writing PCAPNG files with compression
{: .no_toc }

Zstd streaming compression is only supported when working with pcapng files. To enable this feature you must build PcapPlusPlus with Zstd support. For more guidance on building PcapPlusPlus see the [build instructions per platform *LINK*](./install).
...
You can find more information about setting up DPDK and the API in [DPDK support page *LINK*](./dpdk) and also in [DPDK tutorial](./tutorials/dpdk).
...
- IPv4 and IPv6 defragmentation which is a Layer 3 (Network layer) packet reassembly. You can read more about IP fragmentation [here](https://en.wikipedia.org/wiki/IP_fragmentation). To get more information about how it works and the API to use it please refer to the [API documentation *LINK*](./api) and browse through the code of the [IPDefragUtil *LINK*](./examples#ipdefragutil) and [IPFragUtil *LINK*](./examples#ipfragutil) example apps
- TCP reassembly which is a Layer 4 (Transport layer) packet reassembly. To get more information on how it works and the API to use it please refer to the [API documentation *LINK*](./api) and browse through the code of the [TcpReassembly *LINK*](./examples#tcpreassembly) example app
...
You can read more in the [PcapFilter.h API documentation *LINK*](/api-docs/_pcap_filter_8h.html) and in the [capture packets tutorial](./tutorials/capture-packets#filtering-packets).
