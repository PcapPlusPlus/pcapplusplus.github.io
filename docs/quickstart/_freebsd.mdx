...
Before installing PcapPlusPlus make sure you have the prerequisites installed for [FreeBSD *LINK*](/docs/install/build-source/freebsd#prerequisites).

Pre-compiled packages are available for recent versions of FreeBSD. You can find them under the <ReleaseLink/>. After downloading and extracting the archive file go to: `/path/to/your/package/` and run the installation script:

```shell
sudo ./install.sh
```

If you have other versions of FreeBSD you'll need to [build PcapPlusPlus from source *LINK*](/docs/install/build-source/freebsd). Make sure not to skip the [installation *LINK*](/docs/install/build-source/freebsd#installation) part.
