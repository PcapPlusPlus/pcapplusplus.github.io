...
1. `CMake` which can be downloaded from: <https://cmake.org/download/>
...
3. Make sure that MinGW-w64 `bin` directory is in your `PATH` environment variable, for example:
    ...
    ```
3. [WinPcap developer's pack](https://www.winpcap.org/devel.htm) **OR** [Npcap SDK](https://nmap.org/npcap/guide/npcap-devguide.html) - containing the `wpcap` library PcapPlusPlus is linking with plus relevant `.h` files.
...
mkdir build
cd build
cmake .. -G "MinGW Makefiles" -DPCAP_ROOT=<NpcapSDK_or_WinPcapDevPack_Directory>
```

This process will build the following artifacts:
- PcapPlusPlus libs:
  - `build\Common++\libCommon++.dll`
  - `build\Packet++\libPacket++.dll`
  - `build\Pcap++\libPcap++.dll`
