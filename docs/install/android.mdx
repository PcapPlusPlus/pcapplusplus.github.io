...
- The path of Android NDK. Let's assume it's stord in `ANDROID_NDK`
- Android API version - must be between 21 and 30. Let's assume it's stord in `API_VERSIOM`
- Target architecture which accepts the following values: `arm64-v8a`, `armeabi-v7a`, `x86`, `x86_64`. Let's assume it's stord in `ANDROID_ABI`
- The path of `libpcap` compiled for Android. Let's assume it's stord in `LIBPCAP_DIR`
...
When building for Android the build machine is usually different from the target device, so you may want to gather the lib and header files in a directory instead of installing it in the build machine.
